...
`${import.meta.env.BASE_URL}video/qbot/<name>.mp4` — never a bare `/video/...`
path, because the site is served from a sub-path (`/Motion-Website/`) and a
root-relative URL 404s in the build.

Posters go in `public/images/qbot/` as `<same-name>-poster.jpg`.
...
```sh
ffmpeg -ss 2 -i out.mp4 -frames:v 1 -q:v 4 ../../images/qbot/<name>-poster.jpg
```
